...
로 정의한다. 가장 큰 차이점은, 각 단어 y_i마다 독립적인 context vector c_i를 사용한다는 것이다. 
...
\overrightarrow{h_i} =
\begin{cases}
(1 - \overrightarrow{z_i}) \circ \overrightarrow{h_{i-1}} + \overrightarrow{z_i} \circ \overrightarrow{h_i}, & \text{if } i > 0 \\
0, & \text{if } i = 0
\end{cases}

\overrightarrow{h_i} = \tanh(\overrightarrow{W} E x_i + \overrightarrow{U} [\overrightarrow{r_i} \circ \overrightarrow{h_{i-1}}])
